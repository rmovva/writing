...
- `data/original_openings.jsonl`: same metadata plus the opening text.
...
The script randomly chooses pairs where both the original and GPT texts exist, shuffles A/B order, tallies your picks, and then shows the accuracy plus a labeled review of each pair.

## Data snapshot

The repo currently includes a collected sample of 100 openings from famous authors’ less-taught works (see `data/original_openings.jsonl`). Regenerate anytime with the fetch script if you want a different mix.
